...
import { buildActaFinal, buildFinalRankings, buildReport5Rows, buildStats, downloadCsv, fmtAvg, groupStandings, matchCode, matchDetailedScore, matchDisplayStatus, matchPlayerStats, matchRoundKey, matchRoundLabel, playerName, roundDisplayName, getEligiblePlayers } from '../lib/tournament.js';
...
function rankingRow(row) {
  ...
  return E('tr', { key: row.player.player_id, className: shownStatus === 'Clasificado' ? 'qualified-row' : '' },
    ...
    E('td', null, row.caroms), E('td', null, row.innings), E('td', null, row.s1), E('td', null, row.s2), E('td', null, fmtAvg(row.avg)), E('td', null, row.points)
  );
}
...
function matchReportRow(match, playerMap) {
  ...
  return E('tr', { key: match.match_id, className: match.match_status === 'COMPLETED' ? 'completed-row' : '' },
    ...
    E('td', null, `${p1.innings}/${p2.innings}`),
    E('td', null, `${p1.s1}/${p2.s1}`),
    E('td', null, `${p1.s2}/${p2.s2}`),
    E('td', null, `${p1.avg}/${p2.avg}`),
...
export function ReportsModule({ players, matches, groups, seeds, championship = {} }) {
  ...
  const stats = buildStats(scopedMatches, eligiblePlayers);
  const groupsWithStandings = groupStandings(groups, scopedMatches);
  ...
  const performanceRows = [...stats].sort((a, b) => (b.avg || 0) - (a.avg || 0) || b.points - a.points || b.s1 - a.s1 || b.s2 - a.s2);
  ...
  const exportPositions = () => downloadCsv('posiciones_finales.csv', finalRankings.map((row) => ({
    ...
    PJ: row.played, PG: row.wins, PE: row.draws, PP: row.losses, CAR: row.caroms, ENTR: row.innings,
    SM1: row.s1, SM2: row.s2, PROM: fmtAvg(row.avg), PUNTOS: row.points, POS: row.final_position
  })));
  const exportGroups = () => downloadCsv('reporte_grupos.csv', groupsWithStandings.flatMap((g) => g.standings?.map((s) => ({ GRUPO: g.group_name, NUM: s.group_position, 'NOMBRE DEL JUGADOR': playerName(s.player), PJ: s.played, PG: s.wins, PE: s.draws, PP: s.losses, CAR: s.caroms, ENTR: s.innings, SM1: s.s1, SM2: s.s2, PROM: fmtAvg(s.avg), PUNTOS: s.points, POS: s.group_position, CLASIFICADO: seeds.some((seed) => seed.player?.player_id === s.player.player_id) ? 'Clasificado' : 'No clasificado' })) || []));
  const exportBracket = () => downloadCsv('reporte_bracket.csv', eliminationMatches.map((m) => ({
    Ronda: matchRoundLabel(m), Numero_partido: matchCode(m), Seed_1: m.seed1_position || '', Seed_2: m.seed2_position || '', Jugador_1: playerName(playerMap[m.player1_id]), Jugador_2: playerName(playerMap[m.player2_id]), Carambolas_1: m.caroms_p1, Carambolas_2: m.caroms_p2, Entradas: `${m.innings_p1}/${m.innings_p2}`, SM1: `${m.s1_p1}/${m.s1_p2}`, SM2: `${m.s2_p1}/${m.s2_p2}`, Promedio: `${matchPlayerStats(m, 1).avg}/${matchPlayerStats(m, 2).avg}`, Ganador: m.winner_id ? playerName(playerMap[m.winner_id]) : '', Penales: `${m.penalties_p1 || ''}/${m.penalties_p2 || ''}`, Tipo_resultado: m.match_result_type || 'NORMAL'
  })));
  ...
  return E('div', { className: 'grid reports-export-root' },
    ...
    printSections.ranking && E(Card, { className: 'report-standings-card report-page-break' },
      ...
      E('div', { className: 'table-wrap', style: { marginTop: 12 } }, E('table', { className: 'report-standings-table' },
        E('thead', null, E('tr', null, ['POS', 'Jugador', 'Estado', 'Fase', 'PJ', 'PG', 'PE', 'PP', 'CAR', 'ENTR', 'SM1', 'SM2', 'PROM', 'PTS'].map((h) => E('th', { key: h }, h)))),
        E('tbody', null, finalRankings.map(rankingRow))
      ))
    ),
    printSections.bracket && E(Card, { className: 'report-page-break' },
      ...
      E('div', { className: 'table-wrap', style: { marginTop: 12 } }, E('table', { className: 'report-bracket-table' },
        E('thead', null, E('tr', null, ['ID', 'Ronda', 'Jugador 1', 'Jugador 2', 'Marcador', 'Entradas', 'SM1', 'SM2', 'Promedio', 'Estado', 'Ganador', 'Tipo'].map((h) => E('th', { key: h }, h)))),
        E('tbody', null, eliminationMatches.map((m) => matchReportRow(m, playerMap)))
      ))
    ),
    ...
    printSections.firstPhase && E(Card, { className: 'report-standings-card report-page-break first-phase-qualified-report' },
      ...
      E('div', { className: 'table-wrap', style: { marginTop: 12 } }, E('table', { className: 'report-standings-table first-phase-table' },
        E('thead', null, E('tr', null, ['ORDEN', 'GRUPO', 'POS', 'JUGADOR', 'PJ', 'PG', 'PE', 'PP', 'CAR', 'ENTR', 'SM1', 'SM2', 'PROM', 'PUNTOS'].map((h) => E('th', { key: h }, h)))),
    ...
    printSections.performance && E(Card, { className: 'report-standings-card report-page-break' },
      ...
      E('div', { className: 'table-wrap', style: { marginTop: 12 } }, E('table', { className: 'report-standings-table' },
        E('thead', null, E('tr', null, ['POS', 'Jugador', 'PJ', 'PG', 'PE', 'PP', 'CAR', 'ENTR', 'SM1', 'SM2', 'PROM', 'PTS'].map((h) => E('th', { key: h }, h)))),
        E('tbody', null, performanceRows.map((s, i) => E('tr', { key: s.player.player_id },
          E('td', null, i + 1), E('td', null, tablePlayerCell(s.player)), E('td', null, s.played), E('td', null, s.wins), E('td', null, s.draws), E('td', null, s.losses), E('td', null, s.caroms), E('td', null, s.innings), E('td', null, s.s1), E('td', null, s.s2), E('td', null, fmtAvg(s.avg)), E('td', null, s.points)
        )))
      ))
    )
    )
