...
export function GroupsModule({ championship, setChampionship, players, groups, setGroups, matches, setMatches, seeds, setSeeds, audit }) {
  ...
  const replacementPlayers = useMemo(() => players
    ...
    .sort((a, b) => playerName(a).localeCompare(playerName(b))), [players, groupedPlayerIds, championshipPlayerIds]);
      ...
      groups.length ? E('div', { className: 'swap-helper-box group-mutation-box' },
        E('div', { className: 'group-mutation-message' },
          ...
          groupMutationBlockedReason ? E('span', { className: 'validation-text' }, groupMutationBlockedReason)
            : mutationMode === 'swap'
              ? (swapSelection.length ? `Modo intercambio activo: seleccione la segunda línea para intercambiar con ${playerName(swapSelection[0].player)} (${swapSelection[0].group_name}).` : 'Modo intercambio activo: seleccione el primer jugador y luego el segundo jugador. El cambio se aplica de inmediato.')
              : mutationMode === 'substitute'
                ...
                : 'Seleccione primero una acción. Intercambiar y Sustituir son procesos separados para evitar operaciones simultáneas.'
        ),
        E('div', { className: 'toolbar group-mutation-actions' },
          E(Button, { onClick: () => startMutationMode('swap'), kind: mutationMode === 'swap' ? 'success' : 'soft', disabled: Boolean(groupMutationBlockedReason) }, 'Modo intercambio'),
          ...
          mutationMode ? E(Button, { onClick: () => { setMutationMode(''); clearMutationState(); }, kind: 'soft' }, 'Cancelar acción') : null
        ),
