  ...
  return E('header', { className: 'header' },
    E('div', { className: 'brand-block' },
      ...
      E('div', { className: 'brand-copy' },
        E('div', { className: 'brand-title' }, 'ASOBIGRIE'),
        E('div', { className: 'brand-subtitle' }, 'Control de Campeonatos'),
        E('div', { className: 'brand-secondary' }, E('img', { src: '/assets/fecobi-logo.jpg', alt: 'FECOBI' }), E('span', null, 'FECOBI'))
      )
    ),
...
function TopBar({ championship }) {
  return E('div', { className: 'topbar' },
    ...
    E('div', { className: 'topbar-user' },
      ...
      E('div', null, E('b', null, 'Administrador'), E('div', { className: 'small' }, 'FECOBI'))
    )
  );
}
...
function Dashboard({ championship, players, groups, matches, seeds, championships }) {
  ...
  const releaseTitle = `Preproducción v${runningVersion} · Bracket continuo, PDF institucional y ajustes UI`;
  const releaseSummary = `Versión activa v${runningVersion}: gestión de campeonatos múltiples, captura avanzada, Reporte 5, cierre deportivo/administrativo, llaves tabulares y continuas, ranking final y exportaciones institucionales CSV/PDF.`;
