...
-- 4) RLS de profiles: cada usuario solo ve/edita su propia fila;
...
create policy "profiles_select_own" on public.profiles
  for select using (
    id = auth.uid()
    or exists (select 1 from public.profiles p where p.id = auth.uid() and p.role = 'SUPER_USER')
  );
...
-- 5) RLS del estado de aplicación: cada usuario solo accede a su propio blob.
...
-- 6) Auditoría: cada usuario solo inserta sus propios eventos y los lee.
